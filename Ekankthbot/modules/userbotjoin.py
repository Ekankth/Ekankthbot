...
from Ekankthbot import telethn as Ekankth, client
from Ekankthbot.status import *



@Ekankth.on(events.NewMessage(pattern="^[!?/]join ?(.*)"))
@Ekankth.on(events.NewMessage(pattern="^[!?/]userbotjoin ?(.*)"))
@is_admin
async def _(e, perm):
    chat_id = e.chat_id
...
@Ekankth.on(events.NewMessage(pattern="^[!?/]pjoin ?(.*)"))
@is_admin        
async def _(e, perm):
    chat_id = e.chat_id
